...
def _resolve_tickers(args: argparse.Namespace) -> list[str] | None:
    """--tickers and --limit are mutually exclusive at the argparse level.
    --limit takes the first N tickers from config.yaml's configured order --
    a quick way to dev-test against a few tickers without typing out symbols
    or burning NewsAPI's daily request budget against all of them.
    """
    if args.tickers:
        return args.tickers
    if args.limit:
        return [t.symbol for t in get_settings().tickers[: args.limit]]
    return None


def cmd_prices(args: argparse.Namespace) -> None:
    prices_ingest.run(tickers=_resolve_tickers(args), full_refresh=args.full_refresh)


def cmd_news(args: argparse.Namespace) -> None:
    news_ingest.run(tickers=_resolve_tickers(args))
...
def _add_ticker_selection_args(subparser: argparse.ArgumentParser) -> None:
    group = subparser.add_mutually_exclusive_group()
    group.add_argument("--tickers", nargs="+", default=None, help="Subset of tickers to fetch")
    group.add_argument(
        "--limit", type=int, default=None,
        help="Use only the first N configured tickers, in config.yaml's order "
             "(quick dev testing, an alternative to --tickers)",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_ticker_selection_args(p_prices)
